...
Generates content using Amazon Bedrock (Amazon Nova Lite) and saves to S3.
"""
...
def generate_with_bedrock(prompt: str) -> str:
    """Generate content using Amazon Bedrock Nova Lite."""
...
def save_to_s3(
    topic: str, category: str, content: str, meta_description: str, category_name: str
) -> str:
...
def handler(event, context):
    ...
    if invalid_categories:
        return create_response(
            400, {"error": f"Invalid categories: {invalid_categories}"}
        )
    ...
    return create_response(
        200,
        {
            ...
            "message": f"Generated {len(generated)} pages. {len(failed)} failed."
            if failed
            else f"Successfully generated {len(generated)} pages.",
        },
    )
